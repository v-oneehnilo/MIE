...
interface HistoricalCharacter {
  type: 'king' | 'artisan' | 'priest' | 'shepherd' | 'citizen';
  x: number;
  y: number;
  hillType: 'near' | 'mid' | 'far';
  offsetY: number;
...
const drawEngravedGoldLeaf = (
  ...
  glowLevel: number,
  interiorProgress: number = 1.0,
  desaturation: number = 0.0
) => {
  ctx.save();
  ...
  // Helper to mix custom gold color with its silver/grayscale equivalent
  const mixGoldWithSilverRGBA = (rG: number, gG: number, bG: number, factor: number, alpha: number) => {
    const gray = Math.round(0.299 * rG + 0.587 * gG + 0.114 * bG);
    const r = Math.round(rG * (1 - factor) + gray * factor);
    const g = Math.round(gG * (1 - factor) + gray * factor);
    const b = Math.round(bG * (1 - factor) + gray * factor);
  ...
  if (showInterior === 'solid') {
    ...
    goldGrad.addColorStop(0, mixGoldWithSilverRGBA(154, 106, 21, desaturation, 1.0));      // Antique deep bronze-gold
    goldGrad.addColorStop(0.24, mixGoldWithSilverRGBA(245, 158, 11, desaturation, 1.0));   // Saturated amber gold
    goldGrad.addColorStop(0.5, mixGoldWithSilverRGBA(255, 251, 235, desaturation, 1.0));    // Bright white-gold specular highlight
    goldGrad.addColorStop(0.76, mixGoldWithSilverRGBA(251, 191, 36, desaturation, 1.0));   // Glowing yellow gold
    goldGrad.addColorStop(1, mixGoldWithSilverRGBA(120, 53, 15, desaturation, 1.0));       // Deep bronze shadow
    ctx.fillStyle = goldGrad;
    ctx.strokeStyle = mixGoldWithSilverRGBA(251, 191, 36, desaturation, opacity * 0.95);
  } else {
    // Solid opaque backing for golden wireframe lines to stand out beautifully
    ctx.fillStyle = `rgba(10, 24, 18, ${opacity})`;
    // Golden lines
    ctx.strokeStyle = mixGoldWithSilverRGBA(251, 191, 36, desaturation, opacity * 0.95);
  }

  // Gold glow shadow (desaturates to light silver/gray)
  const shadowR = Math.round(245 * (1 - desaturation) + 140 * desaturation);
  const shadowG = Math.round(158 * (1 - desaturation) + 140 * desaturation);
  const shadowB = Math.round(11 * (1 - desaturation) + 140 * desaturation);
  ctx.shadowColor = `rgba(${shadowR}, ${shadowG}, ${shadowB}, ${glowLevel * 0.85})`;
  ...
  // Close the path for precise rendering
  ctx.closePath();
  ctx.fill();
  ctx.stroke();

  // Draw some small rivet circles along the edge (at top, sides, bottom)
  ctx.fillStyle = mixGoldWithSilverRGBA(251, 191, 36, desaturation, opacity * 0.9);
  ...
  if (showInterior === 'engraved') {
    const interiorCol = mixGoldWithSilverRGBA(251, 191, 36, desaturation, opacity * 0.95 * interiorProgress);
    ctx.strokeStyle = interiorCol;
    ctx.fillStyle = interiorCol;
...
export default function App() {
  ...
  const ripplesRef = useRef<{ x: number; y: number; r: number; maxR: number; opacity: number }[]>([]);
  const ritualInteriorProgressRef = useRef(0);
  const leftLeafAlphaRef = useRef(0.0);
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (clickCountRef.current === 3) {
        ...
        if (stage === 'palace') {
          ...
          if (!existingTypes.includes('king')) {
            charactersRef.current.push({
              type: 'king',
              x: width * 0.22,
              y: 0,
          ...
          if (!existingTypes.includes('artisan')) {
            charactersRef.current.push({
              type: 'artisan',
              x: width * 0.82,
              y: 0,
          ...
          if (!existingTypes.includes('priest')) {
            charactersRef.current.push({
              type: 'priest',
              x: width * 0.65,
              y: 0,
        ...
        if (stage === 'reminisce') {
          // Playable Chime Bells Easter Egg at step 16 (礼乐)
          if (reminisceStepRef.current === 16) {
            let clickedBellIdx = -1;
            const pentatonic = [261.63, 293.66, 329.63, 392.00, 440.00];
            for (let bIdx = 0; bIdx < 5; bIdx++) {
              const bx = width * 0.28 + bIdx * (width * 0.44 / 4);
              if (Math.abs(x - bx) < 30 && y >= 60 && y <= 135) {
                clickedBellIdx = bIdx;
                break;
              }
            }
            if (clickedBellIdx !== -1) {
              ...
              return; // Do NOT advance the story step, let them play music!
            }
          }
          ...
          if (nextStep < NARRATIVE_LINES.length) {
            ...
            if (nextStep === 17) {
              charactersRef.current = charactersRef.current.filter(c => c.type !== 'citizen');
              const colors = ['#fed7aa', '#fbcfe8', '#a7f3d0', '#bfdbfe', '#e9d5ff', '#cbd5e1'];
              for (let i = 0; i < 9; i++) {
                const isLeft = Math.random() > 0.5;
                const startX = isLeft ? -50 - i * 35 : width + 50 + i * 35;
                const targetX = width * (0.15 + i * 0.08);
                const hillType = Math.random() > 0.5 ? 'near' : 'mid';
                ...
                charactersRef.current.push({
                  ...
                  y: 0,
                  hillType: hillType,
                  offsetY: hillType === 'near' ? 5 : 8,
                  scale: hillType === 'near' ? 0.85 : 0.72,
                  opacity: 0,
                  bouncePhase: 0,
            ...
            // Step 19 ("属于南越的荣耀。"): Spawn MORE people and MORE sheep walking in!
            if (nextStep === 19) {
              const colors = ['#fde047', '#fed7aa', '#cbd5e1', '#a7f3d0', '#bfdbfe', '#ddd6fe'];
              // Spawn 8 more celebrating citizens walking in
              for (let i = 0; i < 8; i++) {
                const isLeft = Math.random() > 0.5;
                const startX = isLeft ? -40 - i * 30 : width + 40 + i * 30;
                const targetX = width * (0.1 + i * 0.1);
                const hillType = Math.random() > 0.65 ? 'near' : 'mid';
              ...
              // Spawn 6 more sheep walking in from off-screen
              for (let i = 0; i < 6; i++) {
                const isLeft = Math.random() > 0.5;
                const startX = isLeft ? -30 - i * 30 : width + 30 + i * 30;
                const targetX = width * (0.2 + i * 0.12);
                const hillType = Math.random() > 0.5 ? 'near' : 'mid';
                ...
                sheepRef.current.push({
                  ...
                  y: 0,
                  hillType: hillType,
                  offsetY: hillType === 'near' ? 6 : 9,
                  scale: 0.7 + Math.random() * 0.15,
                  facingLeft: !isLeft,
                  bouncePhase: 0,
                  legsSway: 0,
                  opacity: 0,
              ...
              playSheepSound(1.0);
              setTimeout(() => playSheepSound(1.1), 300);
            }
    ...
    const updateAndRender = () => {
      ...
      // --- PROCEDURAL JAGGED LIGHTNING BOLTS (变幻莫测的雷电) ---
      if ((currentStage === 'ritual_outline' || currentStage === 'ritual_interior') && lightningFlashRef.current > 0.6) {
        ctx.save();
        ...
        if (Math.random() > 0.4) {
          ...
          ctx.lineWidth = 1.5;
          ctx.moveTo(lx, ly - 50);
          ctx.lineTo(lx + (Math.random() > 0.5 ? 45 : -45), ly + 15);
          ctx.stroke();
        }
        ctx.restore();
      }

      // --- STORMY RAIN SYSTEM (变幻莫测的骤雨) ---
      if (currentStage === 'ritual_outline' || currentStage === 'ritual_interior') {
        ctx.save();
        ctx.strokeStyle = 'rgba(186, 230, 253, 0.22)'; // Translucent stormy rain streaks
        ctx.lineWidth = 1.0;
        
        // Initialize simple raindrops array on window object for persistence in animation loop
        if (!(window as any).raindrops) {
          (window as any).raindrops = [];
          for (let rIdx = 0; rIdx < 80; rIdx++) {
            (window as any).raindrops.push({
              x: Math.random() * width,
              y: Math.random() * height,
        ...
        const raindrops = (window as any).raindrops;
        raindrops.forEach((drop: any) => {
          drop.y += drop.speed;
          drop.x += drop.speed * 0.12; // slanted
          ...
          ctx.lineTo(drop.x + drop.speed * 0.12, drop.y + drop.len);
          ctx.stroke();
        });
        ctx.restore();
      }

      // --- FLOATING WARM SKY LANTERNS (step 17: 繁华的城池) ---
      if (currentStage === 'reminisce' && reminisceStepRef.current === 17) {
        ctx.save();
        // Keep simple lanterns list on window
        if (!(window as any).lanterns) {
          (window as any).lanterns = [];
          for (let lIdx = 0; lIdx < 16; lIdx++) {
            (window as any).lanterns.push({
              x: Math.random() * width,
        ...
        const lanterns = (window as any).lanterns;
        lanterns.forEach((lantern: any) => {
          lantern.y -= lantern.speedY;
          const lx = lantern.x + Math.sin(time * lantern.freq + lantern.phase) * lantern.amplitude;
          ...
          // Draw warm lantern with glow
          ctx.fillStyle = '#f97316'; // deep orange
          ctx.shadowColor = '#f59e0b';
          ctx.shadowBlur = 10;
          ctx.beginPath();
          ctx.roundRect(lx - lantern.size * 0.5, lantern.y - lantern.size * 0.7, lantern.size, lantern.size * 1.3, 2);
          ctx.fill();
          
          // bottom fire
          ctx.fillStyle = '#fef08a'; // bright yellow core
          ctx.beginPath();
          ctx.arc(lx, lantern.y + lantern.size * 0.5, lantern.size * 0.25, 0, Math.PI * 2);
          ctx.fill();
        });
        ctx.restore();
      }

      // --- GOLDEN LIGHT BEAMS (step 19: 属于南越的荣耀) ---
      if (currentStage === 'reminisce' && reminisceStepRef.current === 19) {
        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        
        for (let bIdx = 0; bIdx < 6; bIdx++) {
          const bx = width * (0.15 + bIdx * 0.14);
          const beamW = 25 + Math.sin(time * 0.05 + bIdx) * 10;
          const alpha = 0.15 + Math.sin(time * 0.03 + bIdx) * 0.08;
          
          const beamGrad = ctx.createLinearGradient(bx, height, bx, 0);
          beamGrad.addColorStop(0, `rgba(251, 191, 36, ${alpha})`);
          beamGrad.addColorStop(0.5, `rgba(245, 158, 11, ${alpha * 0.6})`);
          beamGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
          
          ctx.fillStyle = beamGrad;
          ctx.beginPath();
          ctx.moveTo(bx - beamW, height);
          ctx.lineTo(bx + beamW, height);
          ctx.lineTo(bx + beamW * 0.4, 0);
          ctx.lineTo(bx - beamW * 0.4, 0);
          ctx.closePath();
          ctx.fill();
        }
      ...
      // --- INTERACTIVE BIANZHONG CHIME BELLS (step 16: 礼乐) ---
      if (currentStage === 'reminisce' && reminisceStepRef.current === 16) {
        ctx.save();
        // Wood frame
        ...
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(width * 0.2, 70);
        ctx.lineTo(width * 0.8, 70);
        ctx.stroke();
        
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(width * 0.2, 70);
        ctx.lineTo(width * 0.2, 140);
        ctx.moveTo(width * 0.8, 70);
        ctx.lineTo(width * 0.8, 140);
        ctx.stroke();
        
        const bellNames = ["宫", "商", "角", "徵", "羽"];
        for (let bIdx = 0; bIdx < 5; bIdx++) {
          const bx = width * 0.28 + bIdx * (width * 0.44 / 4);
          const by = 70;
          const swing = Math.sin(time * 0.035 + bIdx) * 0.07;
          ...
          // Bell body
          ctx.fillStyle = '#78350f'; // bronze
          ctx.strokeStyle = '#fbbf24'; // gold lining
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.moveTo(-11, 16);
          ctx.lineTo(-13, 38);
          ctx.quadraticCurveTo(0, 41, 13, 38);
          ctx.lineTo(11, 16);
          ctx.closePath();
          ctx.fill();
          ctx.stroke();
          ...
          ctx.textAlign = 'center';
          ctx.fillText(bellNames[bIdx], 0, 31);
          ctx.restore();
        }
        ctx.restore();
      }

      // --- UPDATE AND DRAW SOUND RIPPLES ---
      if (ripplesRef.current.length > 0) {
        ...
        ripplesRef.current.forEach((rip) => {
          ...
          rip.opacity -= 0.015;
          if (rip.opacity > 0.01) {
            ctx.strokeStyle = `rgba(251, 191, 36, ${rip.opacity * 0.75})`;
            ctx.lineWidth = 2.0;
            ctx.beginPath();
      ...
      const drawMerchantShip = (ctx: CanvasRenderingContext2D, sx: number, sy: number, scaleFactor: number = 1.0) => {
        ctx.save();
        ctx.globalAlpha = pastureOpacity * 0.8 * scaleFactor;
        ctx.fillStyle = '#0a1410'; 
        ctx.strokeStyle = `rgba(251, 191, 36, ${pastureOpacity * 0.85})`; 
        ctx.lineWidth = 1.3;

        ctx.translate(sx, sy);
        ctx.scale(0.55 * scaleFactor, 0.55 * scaleFactor); 
      ...
      if (pastureOpacity > 0.001) {
        ...
        const rStep = reminisceStepRef.current;
        const showShips = storyStageRef.current === 'reminisce' && ((rStep >= 7 && rStep <= 13) || rStep === 19 || rStep === 20);
        if (showShips) {
          shipXRef.current += 0.85;
          if (shipXRef.current > width + 140) {
            shipXRef.current = -140;
          }
          
          // Ship 1: Main merchant ship (Scale 1.0)
          const sx1 = shipXRef.current;
          const sy1 = farY(sx1) - 10;
          drawMerchantShip(ctx, sx1, sy1, 1.0);

          // Ship 2: Following ship, slightly smaller and offset (Scale 0.75)
          const sx2 = sx1 - 100;
          const normSx2 = sx2 < -140 ? width + 140 + (sx2 + 140) : sx2;
          const sy2 = farY(normSx2) - 6;
          drawMerchantShip(ctx, normSx2, sy2, 0.75);

          // Ship 3: Leading background ship, even smaller (Scale 0.55)
          const sx3 = sx1 + 130;
          const normSx3 = sx3 > width + 140 ? -140 + (sx3 - (width + 140)) : sx3;
          const sy3 = farY(normSx3) - 3;
          drawMerchantShip(ctx, normSx3, sy3, 0.55);

          // Spawn thematic cargo particles floating upwards!
          if (Math.random() < 0.08) {
            let color = 'rgba(251, 191, 36, 0.9)'; // default gold
            let isScrollVal = false;
            
            if (rStep === 10) { // Spices
              color = 'rgba(232, 121, 249, 0.9)'; // purple aromatic cloud
            } else if (rStep === 11) { // Jade
              color = 'rgba(110, 231, 183, 0.95)'; // emerald green
            } else if (rStep === 12) { // Glass
              color = 'rgba(34, 211, 238, 0.9)'; // sky blue translucent
            } else if (rStep === 13) { // Stories
              color = 'rgba(253, 230, 138, 0.95)'; // golden scroll
              isScrollVal = true;
            } else if (rStep === 19 || rStep === 20) {
              // Glory part has white-gold celebration sparks!
              color = Math.random() > 0.5 ? 'rgba(251, 191, 36, 0.95)' : 'rgba(254, 243, 199, 0.9)';
            }
            
            particlesRef.current.push({
              x: sx1 + (Math.random() - 0.5) * 20,
              y: sy1 - 8,
              vx: (Math.random() - 0.5) * 0.4 - 0.3, // drift slightly leftwards
              vy: -0.5 - Math.random() * 0.8, // drift upwards
              size: isScrollVal ? 4.0 : 2.0 + Math.random() * 2.0,
              alpha: 1.0,
              decay: 0.01 + Math.random() * 0.01,
              color: color,
              isScroll: isScrollVal
            });
          }
        }
        ...
        sheepRef.current.forEach((sheep, i) => {
          ...
          if (sheep.isBackgroundFlock && sheep.speedX) {
            ...
            // Standard or targetX walking-in behavior
            if (sheep.targetX !== undefined) {
              const diff = sheep.targetX - sheep.x;
              if (Math.abs(diff) > 4) {
                sheep.facingLeft = diff < 0;
                sheep.legsSway += 0.16;
                sheep.x += (diff > 0 ? 1 : -1) * 0.72;
              } else {
                sheep.legsSway = 0;
                // arrived! clear targetX to enter normal grazing behavior
                delete sheep.targetX;
              }
            } else if (sheep.eatingTimer > 0) {
              sheep.eatingTimer--;
              sheep.legsSway = 0;
            } else {
          ...
          // Sheep bleating / jumping in ritual_interior stage
          if (storyStageRef.current === 'ritual_interior') {
            // Force jump bounce periodically
            if (sheep.bouncePhase === 0 && Math.random() < 0.012) {
              sheep.bouncePhase = Math.PI;
            }
            
            // Draw a cute text balloon above head
            ctx.save();
            ctx.translate(10, -32); // above head
            ctx.scale(sheep.facingLeft ? -1 : 1, 1); // undo horizontal scaling
            ctx.fillStyle = 'rgba(254, 243, 199, 0.95)';
            ctx.strokeStyle = '#d97706';
            ctx.lineWidth = 1.0;
            ctx.beginPath();
            ctx.roundRect(-24, -18, 48, 15, 4);
            ctx.fill();
            ctx.stroke();
        ...
        charactersRef.current.forEach((c) => {
          // --- DYNAMIC HISTORICAL STORY ANIMATIONS & WALKING ---
          const rStep = reminisceStepRef.current;
          const currentStage = storyStageRef.current;

          if (currentStage === 'reminisce') {
            if (c.type === 'king') {
              if (rStep === 1 || rStep === 2) {
                // King climbs the Nanyue Palace! Move towards center mid hill
                const targetX = width * 0.52 - 15;
                c.x += (targetX - c.x) * 0.035;
                c.hillType = 'mid';
                // ascend steps height as he walks closer!
                c.offsetY = 6 + 10 * Math.max(0, Math.min(1, (c.x - width * 0.22) / (width * 0.28)));
              } else if (rStep >= 3) {
                // King is at the palace entrance
                c.x = width * 0.52 - 15;
                c.hillType = 'mid';
                c.offsetY = 16;
              }
              ...
              if (rStep >= 4 && rStep <= 6) {
                // Artisan approaches Near hill center to shape gold
              ...
              if (rStep >= 14 && rStep <= 15) {
                // Priest moves to ritual center on Mid Hill
                const targetX = width * 0.65;
                c.x += (targetX - c.x) * 0.04;
                c.hillType = 'mid';
                c.offsetY = 12;
              }
            } else if (c.type === 'citizen') {
              if (c.targetX !== undefined) {
                const diff = c.targetX - c.x;
                if (Math.abs(diff) > 2) {
                  c.x += diff * 0.03; // smooth elegant walk in
                  c.bouncePhase = (c.bouncePhase || 0) + 0.15;
                  c.offsetY = (c.hillType === 'near' ? 5 : 8) + Math.abs(Math.sin(c.bouncePhase)) * 3.5;
                } else {
                  c.bouncePhase = 0;
                  c.offsetY = c.hillType === 'near' ? 5 : 8;
                }
              }
            }
          } else if (currentStage === 'ritual_outline' || currentStage === 'ritual_interior' || currentStage === 'ritual_final') {
            // Positions locked for active rituals
            if (c.type === 'king') {
              c.x = width * 0.52 - 15;
              c.hillType = 'mid';
              c.offsetY = 16;
          ...
          if (c.type === 'shepherd') {
            ...
            // Hammer with swinging animation
            const isHammering = (currentStage === 'reminisce' && rStep >= 4 && rStep <= 6) || (currentStage === 'ritual_interior');
            
            ctx.save();
            ctx.translate(-5, -12); // pivot at artisan's shoulder/hand
            if (isHammering) {
              const hammerAngle = Math.sin(time * 0.16) * 0.9 - 0.45;
              ctx.rotate(hammerAngle);
              
              // Spark & sound trigger on strike
              const swing = Math.sin(time * 0.16);
              if (swing > 0.88 && !c.hasStruck) {
                c.hasStruck = true;
                const strikeX = c.x - 14;
                const strikeY = drawY - 4;
                playChimeBellSound(523.25); // high metallic ping!
                
                // Spawn beautiful physics gold sparks
                for (let sp = 0; sp < 12; sp++) {
                  const angle = -Math.random() * Math.PI; // fly upwards
            ...
            // Purple magic ritual circle and violet particles
            if (rStep === 15 || currentStage === 'ritual_outline' || currentStage === 'ritual_interior') {
              ctx.save();
              ctx.strokeStyle = 'rgba(168, 85, 247, 0.45)';
              ctx.lineWidth = 1.5;
              const phase = (time * 0.015) % 1;
              ctx.beginPath();
              ctx.arc(0, 0, 10 + phase * 65, 0, Math.PI * 2);
              ctx.stroke();
              ctx.restore();
              ...
              if (Math.random() < 0.06) {
                particlesRef.current.push({
                  ...
                  color: '#d946ef' // Violet magic sparkle
                });
              }
            }

            // Chanting Speech bubble during ritual_outline
            if (currentStage === 'ritual_outline') {
              ctx.save();
              ctx.fillStyle = 'rgba(253, 244, 255, 0.95)';
              ctx.strokeStyle = '#a21caf';
              ctx.lineWidth = 1.2;
              ctx.beginPath();
              ctx.roundRect(-80, -95, 160, 24, 6);
              ctx.fill();
              ctx.stroke();
              ...
              ctx.font = 'bold 10px sans-serif';
              ctx.textAlign = 'center';
              ctx.fillText("百越祭礼，金华初现！", 0, -79);
              ctx.restore();
            }
          } else if (c.type === 'citizen') {
            // Citizen styling: humble organic earth colors, simple attire, NO name label above head
            ctx.fillStyle = c.color || '#fed7aa'; // randomized or custom organic citizen colors
            ctx.strokeStyle = '#2d3748'; // dark slate outlines
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(-6, 0);
            ctx.lineTo(-4, -20);
            ...
            // Simple hair bun / topknot
            ctx.fillStyle = '#1a202c';
            ctx.beginPath();
            ctx.arc(0, -29.5, 2, 0, Math.PI * 2);
            ctx.fill();
          }
      ...
      if (leaf) {
        ...
        if (landProgress === 1.0 && clickCountVal === 3 && clickElapsed >= 3500 && storyStageRef.current === 'summoning') {
          ...
          } else if (currentStage === 'reminisce' && reminisceStepRef.current >= 4) {
            // Float back to the left side of the screen with a majestic, slow-drifting wobble
            const targetX = width * 0.18;
            const targetY = height * 0.36;
            const floatOffset = Math.sin(time * 0.02) * 14;
            
            leaf.x += (targetX - leaf.x) * 0.035;
            leaf.y += (targetY + floatOffset - leaf.y) * 0.035;

            // artistic tilt angle (-0.2 radians)
            const targetAngle = -0.2;
            const diff = targetAngle - leaf.angle;
            const wrappedDiff = Math.atan2(Math.sin(diff), Math.cos(diff));
            leaf.angle += wrappedDiff * 0.04;

            // keep flat-facing the viewer
            leaf.rollPhase += (0 - leaf.rollPhase) * 0.05;
            leaf.pitchPhase += (0 - leaf.pitchPhase) * 0.05;
          } else {
        ...
        if (stage === 'ritual_outline' || stage === 'ritual_interior' || stage === 'ritual_final' || stage === 'curtain') {
          ...
          scaleX = 1.35;
          scaleY = 1.35;
        } else if (stage === 'reminisce' && reminisceStepRef.current >= 4) {
          // Giant scale, comparable to Nanyue palace (h=140, w=260)
          scaleX = 3.2;
          scaleY = 3.2;
        } else if (landProgress === 1.0 && clickCountVal === 3 && clickElapsed >= 3500 && stage === 'summoning') {
          const windElapsed = clickElapsed - 3500;
        ...
        // Smoothly animate the alpha of the giant left leaf
        let targetLeftAlpha = 0.0;
        if (stage === 'reminisce' && reminisceStepRef.current >= 4) {
          targetLeftAlpha = 0.85; // golden outline, semi-transparent base
        }
        leftLeafAlphaRef.current += (targetLeftAlpha - leftLeafAlphaRef.current) * 0.045;

        // Determine leaf alpha based on stage and reminisce step
        let leafAlpha = 0.95;
        if (stage === 'reminisce') {
          if (reminisceStepRef.current >= 4) {
            leafAlpha = leftLeafAlphaRef.current;
          } else {
            leafAlpha = 0.95; // resting on the grass
          }
        } else if (stage === 'shepherd' || stage === 'characters' || stage === 'palace') {
          leafAlpha = 0.95; // resting on the grass
        } else if (stage === 'ritual_outline') {
          leafAlpha = 0.95; // fully visible at center during active forging
        }
        ...
        if (leafAlpha > 0.01) {
          ...
          drawEngravedGoldLeaf(ctx, size, leafAlpha, drawMode, glowLevel, ritualInteriorProgressRef.current, desaturation);
        }
      ...
      if (particlesRef.current.length > 0) {
        ...
        particlesRef.current.forEach((spark) => {
          ...
          if (spark.alpha > 0.01) {
            ...
            if (spark.isScroll) {
              // Draw a tiny cute rolled-up scroll
              ctx.save();
              ctx.translate(spark.x, spark.y);
              ctx.rotate(time * 0.05); // rotate gently
              ctx.lineWidth = 1;
              ctx.strokeStyle = '#d97706';
              // Scroll body
